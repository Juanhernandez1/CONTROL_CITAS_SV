...
export default class RequestBusiness {
  ...
  constructor(businessCrud, contactbusinessCrud, addressCrud, freedayCrud, settingCrud, servicesCrud, appointmentCrud) {
  ...
  RequestBusinessCreate = async (req, res) => {
    try {
      ...
      if (ContactFindEmail.data === null) {
        ...
        res.status(409).send(ContactFindEmail.success ? { ...ContactFindEmail,
          data: 'mail exists'
        } : ContactFindEmail);
      }
    } catch (error) {
  ...
  RequestBusinessUpdate = async (req, res) => {
    try {
      ...
      if (Business.data[0] === 1) res.status(202).send(Business);else res.status(406).send({ ...Business,
        success: false
      });
    } catch (error) {
      const {
  ...
  RequestBusinessSettingUpdate = async (req, res) => {
    try {
      ...
      if (BusinessSetting.data[0] === 1) res.status(202).send(BusinessSetting);else res.status(406).send({ ...BusinessSetting,
        success: false
      });
    } catch (error) {
      const {
  ...
  RequestBusinessAddressUpdate = async (req, res) => {
    try {
      ...
      if (BusinessAddess.data[0] === 1) res.status(202).send(BusinessAddess);else res.status(406).send({ ...BusinessAddess,
        success: false
      });
    } catch (error) {
      console.log(error);
  ...
  RequestBusinessContactUpdate = async (req, res) => {
    try {
      ...
      if (BusinessContact.data[0] === 1) res.status(202).send(BusinessContact);else res.status(406).send({ ...BusinessContact,
        success: false
      });
    } catch (error) {
      const {
